...
void Sight::setPlayer(Player *p) {
	this->owner = p;
	this->sprite.setPosition(this->owner->getTankPos() - sf::Vector2f (0, this->sprite.getLocalBounds().width));
}
...
void Sight::move() {
	...
	sf::Vector2f turretPosition = this->owner->getTurretPos();
	...
	this->owner->setTurretAngle(findThePolarAngleOfTheVector(vectorTurretCursor.x, vectorTurretCursor.y) * 180 / M_PI);
}
